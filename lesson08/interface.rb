...
class Interface
  ...
  def prompt_wagon_number_of_seats
    print 'Введите количество пассажирских мест: '
  end

  def prompt_wagon_capacity
    print 'Введите грузовой объем вагона: '
  end
  ...
  def prompt_capacity_occupy_select
    print 'Введите занимаемый обЪем: '
  end

  def prompt_capacity_free_select
    print 'Введите освобождаемый обЪем: '
  end
  ...
  def show_train_wagons(train)
    train.wagons.each { |wagon| puts wagon.str.rjust(12)}
  end

  private

  def show_train(train, index)
    puts index.to_s.rjust(2) + " - Номер поезда: #{train.number}, тип: #{train.type}, кол.вагонов: #{train.wagons.length}, маршрут: "
