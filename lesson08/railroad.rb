...
class Railroad
  ...
  def hook_train_wagon(train, wagon_number, wagon_capacity)
    wagon = train.wagons.select { |wagon| wagon.number == wagon_number}.pop
    raise StandardError.new("Уже есть вагон с номером #{wagon.number}") unless wagon.nil?
    wagon = PassengerWagon.new(wagon_number, wagon_capacity) if train.type == :passenger
    wagon = CargoWagon.new(wagon_number, wagon_capacity) if train.type == :cargo
  ...
  def occupy_seat_in_wagon(wagon, capacity)
    if capacity.nil?
      wagon.occupy
    else
  ...
  def free_seat_in_wagon(wagon, capacity)
    if capacity.nil?
      wagon.free
    else
      wagon.free(capacity)
